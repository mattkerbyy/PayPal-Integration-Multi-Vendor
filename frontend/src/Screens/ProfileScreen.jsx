  ...
  useEffect(() => {
    if (!userInfo) {
      ...
      fetch("/api/check-seller-group", {
        ...
        .then((res) => res.json())
        .then((data) => {
          console.log("Seller check response:", data); // Log response for debugging
          setIsSeller(data.is_seller); // Ensure correct key is used for seller status
        })
        .catch((error) => {
          console.error("Error checking seller group:", error);
        });
      // Check if user is in Admin group
      fetch("/api/check-admin-group", {
        method: "GET",
        headers: {
          Authorization: `Bearer ${userInfo.token}`,
        },
  ...
  return (
    <>
      <Row>
        ...
        <Col md={9}>
          {isSeller ? (
            <SellerDashboardScreen /> // Render SellerDashboardScreen if user is a seller
          ) : isAdmin ? (
